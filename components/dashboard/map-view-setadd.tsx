...
// -> NUEVO: Interfaz para exponer la ubicación si necesitas pasarla a un formulario padre
...
export function MapView({ onLocationSelect }: MapViewProps) {
  ...
  // -> NUEVO: Referencia para almacenar el marcador del usuario y evitar que se duplique
  ...
  React.useEffect(() => {
    ...
    const handleMapClick = (e: maplibregl.MapMouseEvent) => {
      ...
      // -> NUEVO: 2. Lógica para posicionar el marcador del usuario al hacer click
      const target = e.originalEvent.target as HTMLElement;
      // Evitamos poner un marcador si el usuario hizo click sobre un marcador de precio existente
      if (target.closest(".listing-marker-container")) return;

      const { lng, lat } = e.lngLat;

      // Si ya existía un marcador del usuario, lo removemos antes de crear el nuevo
      if (userMarkerRef.current) {
        userMarkerRef.current.remove();
      }

      // Creamos un elemento DOM personalizado y estilizado con Tailwind para el marcador del usuario
      const userMarkerEl = document.createElement("div");
      userMarkerEl.className = "user-selected-marker";
      userMarkerEl.innerHTML = `
        <div class="relative flex items-center justify-center">
          <div class="absolute w-8 h-8 bg-primary/30 rounded-full animate-ping"></div>
          <div class="w-5 h-5 bg-primary border-2 border-white rounded-full shadow-md flex items-center justify-center">
            <div class="w-2 h-2 bg-white rounded-full"></div>
          </div>
        </div>
      `;

      // Inicializamos el nuevo marcador en las coordenadas del click
      const newUserMarker = new maplibregl.Marker({
        element: userMarkerEl,
        draggable: true, // Hacemos que también se pueda arrastrar si el usuario quiere precisar
      })
        .setLngLat([lng, lat])
        .addTo(map);

      // Si el usuario arrastra el pin, actualizamos las coordenadas
      newUserMarker.on("dragend", () => {
        const newLngLat = newUserMarker.getLngLat();
        if (onLocationSelect) {
          onLocationSelect({ lat: newLngLat.lat, lng: newLngLat.lng });
        }
      });

      userMarkerRef.current = newUserMarker;

      // Notificamos al componente padre (útil para formularios de creación de propiedades)
      if (onLocationSelect) {
    ...
    return () => {
      ...
      map.remove();
      mapRef.current = null;
      // -> NUEVO: Limpieza del marcador del usuario al desmontar
      if (userMarkerRef.current) {
        userMarkerRef.current.remove();
        userMarkerRef.current = null;
      }
    };
  }, [selectListing, onLocationSelect]); // -> NUEVO: Añadida dependencia
